...
const BookPage = async (props: PageProps) => {
    ...
    const { data: bookDetails } = await getBooksDetailsData(book?.id);

    const { data: relatedBookData } = await getRelatedBooksData(book?.id);


    return (
        <div className='m-5 flex'>
            <div className='h-full m-0 w-[65%] bg-[#042c43] border border-[rgba(255,255,255,0.59)] box-border rounded-[20px] mt-12 flex flex-wrap overflow-hidden'>
                <div className='w-full p-0'>
                    <div className='relative aspect-[1.01]'>
                        <span className='box-border block overflow-hidden w-[initial] h-[initial] bg-none opacity-100 m-0 p-0 absolute inset-0'>
                            {
                                bookDetails ?
                                    <Image src={bookDetails.media.originUrl} sizes="100vw" width={1500} height={770} alt='' className='rounded-tl-[20px] rounded-tr-[20px] absolute inset-0 box-border p-0 m-auto block bg-cover' />
                                    :
                                    <></>
                            }
                        </span>
                    ...
                    <div>
                        <div className="px-12.5 pt-12.5 pb-0 items-center  box-border justify-between! flex!">
                            <div className="text-white text-[30px] font-semibold">{bookDetails.title}</div>
                        ...
                        <div className="px-12.5 max-w-full relative w-full min-h-px">
                            <div className="mt-12.5">
                                ...
                                <h6 className="text-[16px]/[24px]! mx-[13px] text-white mt-0 mb-[.5em] font-medium">{bookDetails.aboutTheBook}</h6>
                            </div>
                            <div className="mt-12.5">
                                <h5 className="text-[16px]/[20px]! text-white font-bold mt-0 mb-[.5em]">Ai nên đọc sách này ?</h5>
                                <h6 className="text-[16px]/[24px]! mx-[13px] text-white mt-0 mb-[.5em] font-medium">{bookDetails.whoShouldRead}</h6>
                            </div>
                            <div className="mt-12.5">
                                <h5 className="text-[16px]/[20px]! text-white font-bold mt-0 mb-[.5em]">Ai viết ra cuốn sách này ?</h5>
                                <h6 className="text-[16px]/[24px]! mx-[13px] text-white mt-0 mb-[.5em] font-medium">{bookDetails.aboutTheAuthor}</h6>
                            </div>
                            <div className="mt-12.5">
                                <h5 className="text-[16px]/[20px]! text-white font-bold mt-0 mb-[.5em]">Phụ lục</h5>
                                <div className="mt-6">
                                    {
                                        bookDetails.chapters.map((item, index) => (
            ...
            <div className="w-[35%] m-0! py-8 pr-0 pl-8 flex flex-wrap">
                <div className="container">
                    <div className="flex flex-wrap -mx-[15px]">
                        ...
                        {
                            relatedBookData.slice(0, 3).map((item) => (
                                        ...
                                        <div className="bg-[#242730] p-0 rounded-[20px]">
                                            ...
                                            <div className="text-white h-[153px] w-full pt-5">
                                                ...
                                                <div className="mx-[1.9rem] mt-[5px]">
                                                    <span className="text-ellipsis overflow-hidden whitespace-pre-wrap text-[13px]/[15px] tracking-[.5px] text-[#b7b9d2] mix-blend-normal font-normal">{item.subTitle}</span>
                                                </div>
